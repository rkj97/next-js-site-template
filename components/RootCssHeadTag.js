...
const cssStringFunction = (themeObject, parentAttribute = "") => {
  ...
  for (const attribute in themeObject) {
    cssString =
      typeof themeObject[attribute] === "object"
        ? `${cssString} ${cssStringFunction(
            themeObject[attribute],
            `${parentAttribute}-${attribute}`
          )}`
        : `${cssString}--${parentAttribute}-${attribute}:${themeObject[attribute]};`;
  }
  return cssString;
};
...
const RootCssHeadTag = (props) => {
  return (
    <Head>
      <style>{` ${getRootCssFromJs(props.theme, "theme")} `}</style>
    </Head>
  );
};
